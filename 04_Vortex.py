...
class myGame(TUGame.Game):
    ...
    def frame(self):
        ...
        for i in range(0, 500):
            ...
            f_vortex = np.array([0., 0., 0.]) ##소용돌이

            if self.loc[i][1] < 10:
                f_drag = -C_drag * self.vel[i] # 항력을 계산
                f_buoy = -rho * g * 4.0 #부력을 계산
                x, z = self.loc[i][0], self.loc[i][2]
                rotation = 0.5 * np.array([-z, 0., x]) #회전을 계산 공의 x와 z를 바꾸고 -z를 사용
                gather = 0.3 * np.array([-x, 1., -z]) # 중앙으로 모이게 하기 위해 회전의 x z를 바꾼뒤 x값도 -값을 준다
                f_vortex = rotation + gather #회전과 중앙으로 모이는 값을 더해 소용돌이 생성
            F = f_drag + f_buoy +f_vortex ##물체에 힘이 가해질땐 힘들이 더해진다
